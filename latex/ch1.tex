...
\begin{equation}
    \delta(\symbf{x})=\int\frac{\symrm{d}^3 k}{\left(2\pi\right)^3}\ \symrm{e}^{\symrm{i}\symbf{k\cdot x}}\ \delta(\symbf{k}),
\end{equation}
...
\end{equation}
donde \(\delta(\symbf{k})\) es un campo aleatorio complejo
\begin{equation}
    \delta(\symbf{k})=A_k+\symrm{i}B_k.\label{eq::deltak}
\end{equation}
Por ser el campo de fluctuaciones \(\delta(\symbf{x})\) real, se ha de cumplir
\begin{equation}
    \delta(\symbf{-k})=\delta^{*}(\symbf{k})\Longleftrightarrow A_k=A_{-k}\wedge B_k=-B_{-k}.\label{eq::reality}
\end{equation}
...
\end{equation}
Por la realidad de \(\delta(\symbf{x})\), la función de correlación también	es real, así que \(\symrm{P}(k)\) es una función real de igual manera. Mientras que la función de correlación \(\xi(r)\) es el objeto a considerar en las observaciones, el espectro de potencias \(\symrm{P}(k)\) es más fácil de predecir teóricamente y será usado en las simulaciones de este trabajo. Ambos están relacionados por~\ref{eq::relation} y son, por lo tanto, descripciones completamente equivalentes de la estadística.
...
Se generarán realizaciones del campo de fluctuaciones gaussiano para ambos espectros de potencias, \(\symrm{P}_0(k)\) y \(\symrm{P}(k)\), para diferentes valores de \(n_s\):
\begin{itemize}
    \item \(n_s=0\). Ruido blanco, el fondo sobre el que perturbamos.
